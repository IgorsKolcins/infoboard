...
void WgClock::renderMode1(){
	...
	text->SetSize(oneGridStep.vertical/1.5);
	text->TextMid(BUFTIME, posx + (oneGridStep.horizontal/2),
		posy + (oneGridStep.vertical/1.5/4));
}

void WgClock::renderMode2(){
	text->SetColour(0, 85, 81);
	text->SetSize(oneGridStep.vertical/3);
	text->TextMid(BUFPARA, posx + (oneGridStep.horizontal/2),
				  posy - oneGridStep.vertical/3*2);
}

void WgClock::renderMode3(){
	text->SetSize(oneGridStep.vertical/5);
	text->TextMid(BUFINFO, posx + (oneGridStep.horizontal/2),
				 posy - oneGridStep.vertical - (oneGridStep.vertical/5/2));
	setTextColor(color);
	text->SetSize(oneGridStep.vertical/2.2);
	text->TextMid(BUFTIMER, posx + (oneGridStep.horizontal/2), 
